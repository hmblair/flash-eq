...
% Radial weight table dimensions
\pgfmathsetmacro{\matCenter}{\matX+\C}
\def\tblN{19}
\pgfmathsetmacro{\tblW}{\tblN*\cs}
\pgfmathsetmacro{\tblX}{\matCenter-\tblW/2}
\pgfmathsetmacro{\tblY}{\cs+1.2}

% ── Radial weight table (single row) ─────────────────────────────
\begin{scope}[shift={(\tblX, \tblY)}]
  % m=0: 9 purple cells
  \foreach \i in {0,...,8}{\fill[m0, opacity=0.50] ({\i*\cs}, 0) rectangle ++(\cs, \cs);}
  % m=1: 8 gold cells
  \foreach \i in {9,...,16}{\fill[m1, opacity=0.50] ({\i*\cs}, 0) rectangle ++(\cs, \cs);}
  % m=2: 2 green cells
  \foreach \i in {17,18}{\fill[m2, opacity=0.50] ({\i*\cs}, 0) rectangle ++(\cs, \cs);}
  % Grid and border
  \draw[gray!50, thin] (0,0) grid[step=\cs] ({\tblW}, \cs);
  \draw[gray!70] (0,0) rectangle ({\tblW}, \cs);
  % Group separators
  \draw[gray!70, semithick] ({9*\cs}, 0) -- ({9*\cs}, \cs);
  \draw[gray!70, semithick] ({17*\cs}, 0) -- ({17*\cs}, \cs);
  % m=0 labels
  ...
  \node[font=\tiny, m0!55!black] at (3.5*\cs, 0.5*\cs) {$\lambda_{10}$};
  \node[font=\tiny, m0!55!black] at (4.5*\cs, 0.5*\cs) {$\lambda_{11}$};
  \node[font=\tiny, m0!55!black] at (5.5*\cs, 0.5*\cs) {$\lambda_{12}$};
  \node[font=\tiny, m0!55!black] at (6.5*\cs, 0.5*\cs) {$\lambda_{20}$};
  \node[font=\tiny, m0!55!black] at (7.5*\cs, 0.5*\cs) {$\lambda_{21}$};
  \node[font=\tiny, m0!55!black] at (8.5*\cs, 0.5*\cs) {$\lambda_{22}$};
  % m=1 labels (a's then b's)
  \node[font=\tiny, m1!55!black] at (9.5*\cs, 0.5*\cs) {$a_{11}$};
  \node[font=\tiny, m1!55!black] at (10.5*\cs, 0.5*\cs) {$a_{12}$};
  \node[font=\tiny, m1!55!black] at (11.5*\cs, 0.5*\cs) {$a_{21}$};
  \node[font=\tiny, m1!55!black] at (12.5*\cs, 0.5*\cs) {$a_{22}$};
  \node[font=\tiny, m1!55!black] at (13.5*\cs, 0.5*\cs) {$b_{11}$};
  \node[font=\tiny, m1!55!black] at (14.5*\cs, 0.5*\cs) {$b_{12}$};
  \node[font=\tiny, m1!55!black] at (15.5*\cs, 0.5*\cs) {$b_{21}$};
  \node[font=\tiny, m1!55!black] at (16.5*\cs, 0.5*\cs) {$b_{22}$};
  % m=2 labels
  \node[font=\tiny, m2!55!black] at (17.5*\cs, 0.5*\cs) {$a_{22}$};
  \node[font=\tiny, m2!55!black] at (18.5*\cs, 0.5*\cs) {$b_{22}$};
  % Label
  \node[font=\normalsize, text=txt, above=3pt] at ({9.5*\cs}, \cs) {$\mathbf{w}(r)$};
\end{scope}

% Curved arrows from table groups to corresponding matrix blocks
\draw[->, thick, m0!70!black]
  ({\tblX + 4.5*\cs}, \tblY) to[out=-80, in=120, looseness=0.6]
  ({\matX + 1.5*\cs}, \cs);
\draw[->, thick, m1!70!black]
  ({\tblX + 12.5*\cs}, \tblY) to[out=-80, in=90, looseness=0.6]
  ({\matX + 5*\cs}, {-2*\cs});
\draw[->, thick, m2!70!black]
  ({\tblX + 17.5*\cs}, \tblY) to[out=-100, in=90, looseness=0.6]
  ({\matX + 8*\cs}, {-6*\cs});
